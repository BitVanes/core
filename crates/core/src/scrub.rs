...
fn builtin_config(kind: BuiltInPattern) -> (&'static str, &'static str, Validator) {
    match kind {
        ...
        BuiltInPattern::Ssn => (r"\b\d{3}-\d{2}-\d{4}\b", "[SSN]", Validator::None),
        ...
        BuiltInPattern::AwsKey => (r"\bAKIA[0-9A-Z]{16}\b", "[AWS_KEY]", Validator::None),
...
/// fails to compile.
pub fn scrub_text(text: &str, profile: &ScrubProfile) -> Result<(String, OffsetMap)> {
...
///
/// Returns [`BitVanesError::InvalidConfig`] if any regex fails to compile.
pub fn scrub_document(doc: Document, profile: &ScrubProfile) -> Result<(Document, OffsetMap)> {
    let scrubber = Scrubber::from_profile(profile)?;
...
mod tests {
    ...
    fn scrub_with(text: &str, patterns: &[BuiltInPattern]) -> (String, OffsetMap) {
    ...
    fn phone_e164_is_redacted() {
        let (out, _) = scrub_with("Call +15551234567.", &[BuiltInPattern::Phone]);
        assert_eq!(out, "Call [PHONE].");
    }

    #[test]
    fn aws_key_is_redacted() {
        let (out, _) = scrub_with("key = AKIAIOSFODNN7EXAMPLE", &[BuiltInPattern::AwsKey]);
    ...
    fn github_pat_is_redacted() {
        ...
        let (out, _) = scrub_with(&format!("token: {pat}"), &[BuiltInPattern::GitHubPat]);
    ...
    fn credit_card_without_separators() {
        ...
        let (out, _) = scrub_with("4111111111111111", &[BuiltInPattern::CreditCard]);
    ...
    fn scrub_document_projects_span_offsets() {
        ...
        let (scrubbed_doc, map) = scrub_document(doc.clone(), &profile).expect("scrub");
