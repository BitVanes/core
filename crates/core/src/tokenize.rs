...
impl Tokenizer {
    ...
    /// safety).
    pub fn split_at_token_boundary(
        &self,
        text: &str,
        max_tokens: usize,
    ) -> Result<(usize, usize)> {
        ...
        let prefix_bytes =
            self.bpe.decode_bytes(prefix_tokens).map_err(|e| {
                BitVanesError::InvalidInput(format!("BPE decode failed: {e}"))
            })?;
...
mod tests {
    ...
    fn cl100k() -> Tokenizer {
        Tokenizer::new(TokenizerKind::Cl100kBase)
            .expect("cl100k_base should load")
    }
    ...
    #[test]
    fn o200k_base_loads() {
        let t = Tokenizer::new(TokenizerKind::O200kBase)
            .expect("o200k_base should load");
        assert!(t.count("hello world") > 0);
    }
    ...
    fn all_tokenizer_variants_load_and_count() {
        ...
        for kind in [
            ...
            let t = Tokenizer::new(kind).unwrap_or_else(|e| {
                panic!("tokenizer {kind:?} should load: {e}")
            });
            assert!(
