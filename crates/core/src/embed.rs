...
mod ort_embedder {
    ...
    impl OrtEmbedder {
        ...
        pub fn new(
            ...
            let session = Session::builder()
                ...
                .map_err(|e| {
                    BitVanesError::InvalidInput(format!("ort threads: {e}"))
                })?
    ...
    impl Embedder for OrtEmbedder {
        ...
        fn embed(&self, texts: &[&str]) -> Result<Vec<Vec<f32>>> {
            let mut session = self.session.lock().map_err(|e| {
                BitVanesError::InvalidInput(format!("session lock: {e}"))
            })?;
    ...
    fn embed_one(
        ...
        let encoding = tokenizer.encode(text, true).map_err(|e| {
            BitVanesError::InvalidInput(format!("tokenization: {e}"))
        })?;
        ...
        let (input_ids, attn_mask) =
            pad_or_truncate(ids, attention, max_seq_len);

        // 3. Build input tensors (shape: [1, seq_len])
        let input_ids_arr = Array2::from_shape_vec((1, max_seq_len), input_ids)
            .map_err(|e| {
                BitVanesError::InvalidInput(format!("ndarray shape: {e}"))
            })?;
        let attn_arr =
            Array2::from_shape_vec((1, max_seq_len), attn_mask.clone())
                .map_err(|e| {
                    BitVanesError::InvalidInput(format!("ndarray shape: {e}"))
                })?;

        // 4. Run inference
        let ids_tensor = Tensor::from_array(input_ids_arr).map_err(|e| {
            BitVanesError::InvalidInput(format!("input_ids tensor: {e}"))
        })?;
        let attn_tensor = Tensor::from_array(attn_arr).map_err(|e| {
            BitVanesError::InvalidInput(format!("attention_mask tensor: {e}"))
        })?;
        let outputs = session
            .run(inputs![
            ...
            .map_err(|e| {
                BitVanesError::InvalidInput(format!("ort inference: {e}"))
            })?;
        ...
        let hidden = outputs["last_hidden_state"]
            ...
            .map_err(|e| {
                BitVanesError::InvalidInput(format!("tensor extract: {e}"))
            })?;
