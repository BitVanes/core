        ...
        match event {
            ...
            Event::End(TagEnd::Heading(_)) => {
                if let Some(level) = self.pending_heading_level.take() {
                    ...
                    let heading =
                        std::mem::take(&mut self.pending_heading_text);
            ...
            Event::Start(Tag::Paragraph) => {
                if self.in_list_item {
                    ...
                    if self.block_kind.is_some()
                        && !self.full_text.ends_with('\n')
                    {
...
mod tests {
    ...
    fn heading_stack_truncates_correctly_on_sibling_headings() {
        ...
        let paths: Vec<&Vec<String>> =
            doc.spans.iter().map(|s| &s.heading_path).collect();
    ...
    fn inline_formatting_is_stripped_to_plain_text() {
        let doc = parse_md(
            "This is **bold**, *italic*, and [a link](https://x.com).",
        );
        assert_eq!(doc.spans.len(), 1);
        assert_eq!(
            doc.spans[0].text(&doc),
