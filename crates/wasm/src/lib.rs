...
pub fn process(config_js: JsValue, bytes: &[u8]) -> Result<u32, JsValue> {
    let cfg: bitvanes_core::PipelineConfig =
        serde_wasm_bindgen::from_value(config_js).map_err(|e| {
            JsValue::from_str(&format!("config deserialization failed: {e}"))
        })?;
...
pub fn array_ptr(slot_id: u32) -> u32 {
    bitvanes_core::arrow_io::ffi::ffi_pointers(slot_id)
        .map_or(0, |(array, _)| array as u32)
}
...
pub fn schema_ptr(slot_id: u32) -> u32 {
    bitvanes_core::arrow_io::ffi::ffi_pointers(slot_id)
        .map_or(0, |(_, schema)| schema as u32)
}
...
pub fn process_chunks(
    ...
    let (scrubbed, _) =
        bitvanes_core::scrub::scrub_document(doc, &cfg.scrub)
            .map_err(|e| JsValue::from_str(&format!("scrub: {e}")))?;
    let chunks = bitvanes_core::chunk::chunk_document(
        &scrubbed,
        &cfg.chunk,
